...
func toggle_grid_fill_placement():
	placement_mode = GapPlacementMode.GridFillPlacement.new(_last_plane_options)
...
func _select_axis(axis: Vector3):
	...
	var plane_placement = (
		placement_mode is GapPlacementMode.PlanePlacement
		or placement_mode is GapPlacementMode.GridFillPlacement
	)
	if plane_placement and (idle_mode || movement_mode):
		_last_plane_options.normal = axis.normalized()
		if placement_mode is GapPlacementMode.GridFillPlacement:
			placement_mode = GapPlacementMode.GridFillPlacement.new(_last_plane_options)
		else:
			placement_mode = GapPlacementMode.PlanePlacement.new(_last_plane_options)
...
func move_plane_up(direction: int):
	if (
		...
		if placement_mode is GapPlacementMode.GridFillPlacement:
			plane_options = placement_mode.plane_options
		else:
			plane_options = placement_mode.plane_options
		...
		if options.snapping_enabled:
			...
			var snapped_distance = (
				round(distance_along_normal / options.snapping_grid_step) * options.snapping_grid_step
			)
			new_origin = normal * snapped_distance

		plane_options.origin = new_origin
		if placement_mode is GapPlacementMode.GridFillPlacement:
			placement_mode = GapPlacementMode.GridFillPlacement.new(plane_options)
		else:
