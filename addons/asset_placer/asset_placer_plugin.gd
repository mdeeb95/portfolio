...
func _forward_3d_gui_input(viewport_camera, event):
	...
	if event is InputEventKey and event.is_pressed():
		if event.keycode == KEY_ESCAPE:
			if _asset_placer.is_grid_fill_mode():
				_asset_placer.cancel_grid_fill()
			_presenter.cancel()
			return _handled()
	...
	if _asset_placer.is_grid_fill_mode():
		if event is InputEventMouseMotion:
			if event.button_mask & MOUSE_BUTTON_MASK_LEFT:
				if _asset_placer.update_grid_fill(viewport_camera, event.position):
					return _handled()
		...
		if event is InputEventMouseButton:
			if event.button_index == MOUSE_BUTTON_RIGHT:
				...
				if event.pressed:
					if _asset_placer.begin_grid_fill(viewport_camera, event.position):
						return _handled()
				elif _asset_placer.commit_grid_fill(Input.is_key_pressed(KEY_SHIFT)):
					return _handled()
		return EditorPlugin.AFTER_GUI_INPUT_PASS
