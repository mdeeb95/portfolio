...
func _ready():
	...
	presenter.asset_selected.connect(
		func(a):
			if (
				presenter.placement_mode is GapPlacementMode.PlanePlacement
				or presenter.placement_mode is GapPlacementMode.GridFillPlacement
			):
				show()
	)
...
func _react_placement_mode_change(placement_mode: GapPlacementMode):
	if (
		...
		if placement_mode is GapPlacementMode.GridFillPlacement:
			...
			plane_options = placement_mode.plane_options
		_update_mes_per_plane_configuration(plane_options)
	else:
		hide()
