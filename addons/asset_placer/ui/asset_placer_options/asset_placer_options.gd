...
func _ready():
	...
	placement_mode_options_button.item_selected.connect(
		func(id):
			match id:
				...
				2:
					presenter.toggle_grid_fill_placement()
				3:
	...
	plane_axis_spin_box.value_changed.connect(
		func(normal: Vector3):
			...
			if presenter.placement_mode is GapPlacementMode.GridFillPlacement:
				presenter.placement_mode = GapPlacementMode.GridFillPlacement.new(plane)
			else:
...
func show_placement_mode(mode: GapPlacementMode):
	if mode is GapPlacementMode.PlanePlacement or mode is GapPlacementMode.GridFillPlacement:
		plane_axis_container.show()
		plane_origin_container.show()
		var plane_options: PlaneOptions
		if mode is GapPlacementMode.GridFillPlacement:
			placement_mode_options_button.select(2)
			plane_options = mode.plane_options
		else:
			placement_mode_options_button.select(1)
			plane_options = mode.plane_options
		plane_axis_spin_box.set_value_no_signal(plane_options.normal)
		plane_origin_spin_box.set_value_no_signal(plane_options.origin)
	else:
	...
	if mode is GapPlacementMode.Terrain3DPlacement:
		placement_mode_options_button.select(3)
